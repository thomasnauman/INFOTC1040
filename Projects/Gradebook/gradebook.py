...
average_by_assignment = []
average_by_student = []


def student():
    for each_row in gradebook:
        total = 0
        for column in each_row:
            total += column
        avg = total / len(each_row)
        ravg = round(avg, 2)
        average_by_student.append(ravg)


def assignment():
    i = 0
    for _ in gradebook[0]:
        assignment = [row[i] for row in gradebook]
        total = 0
        for grade in assignment:
            total += grade
        avg = total / len(assignment)
        average_by_assignment.append(avg)
        i += 1
...
def displayer():
    ...
    for i in range(len(gradebook)):
        print(f"The average score for Student {i+1} was {average_by_student[i]}")
    print("\n")
    print("Assignment Averages")
    for i in range(len(gradebook[0])):
        print(f"The average score on Assignment {i+1} was {average_by_assignment[i]}")
